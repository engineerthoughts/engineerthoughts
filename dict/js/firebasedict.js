function engtogerfun() 
{
  ...
  for(inc=0;inc<=Englishworddic.length-1;inc++)
  {
	...
	gobuttonid.addEventListener('Click',(e) => {
	
		e.preventDefault();	
		database.ref('input'+inputVal.value).set
		({
		englishvalue: Englishworddic[outpval].value,
		germanvalue: Germanworddic[outpval].value
		});
	});*/
